...
def stock_screen():
    ...
    graph_label.pack()
    Label(screen,text="",bg=colour).pack()
    ok_button=tk.Button(screen,text="Ok",command=graph,height=1,width=10,font="Calibi 16 bold")
    ok_button.pack()
...
def register_user(username, password) :
    ...
    if username=="" or password=="":
        ...
        with open("Credentials.txt", "a") as f:
            ...
            print("Saved")
            Label(screen,text="Registration Success",font="Calibri 12 bold",fg="green",bg=colour).pack()
...
def check_unique(username, password):
    ...
    with open("Credentials.txt", "r") as f:
        ...
        if username_txt not in existing_usernames:
...
def register() :
    ...
    Label(screen,text="",bg=colour).pack()
    Label(screen,text="Password *",font="Calibri 16 bold",bg=colour).pack()
    password_entry=Entry(screen,textvariable=password)
    password_entry.pack()
    Label(screen,text="",bg=colour).pack()
    Label(screen,text="",bg=colour).pack()
    Button(screen,text="Register",font="Calibri 16 bold",width=10,height=1,command=lambda: check_unique(username, password)).pack()
    Label(screen,text="",bg=colour).pack()
    Label(screen,text="",bg=colour).pack()
    Button(screen,text="Back",font="Calibri 16 bold",width=10,height=1,command=back_mainscreen).pack()
